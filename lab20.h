...
Equipment::Equipment(int adhp,int adatk,int addef){
	hpmax = adhp;
	atk = adatk;
	def = addef;
}
...
void Unit::equip(Equipment *ad){
	...
	hpmax += ad->getStat()[0]-equipment->getStat()[0];
	atk += ad->getStat()[1]-equipment->getStat()[1];
	def += ad->getStat()[2]-equipment->getStat()[2];
	}
	equipment = ad;
	if (hp > hpmax)
	{
